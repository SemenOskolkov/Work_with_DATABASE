        ...
        with open('customers_data.csv') as file:  # Работа с файлом customers_data.csv
            ...
            for row in customers_file:
                cur.execute("INSERT INTO customers VALUES (%s, %s, %s)",
                            (row['customer_id'], row['company_name'], row['contact_name']))  # Заполнение данных в таблицу
        ...
        with open('orders_data.csv') as file:  # Работа с файлом orders_data.csv
            ...
            for row in orders_file:
                cur.execute("INSERT INTO orders VALUES (%s, %s, %s, %s, $s)", (
                    row['order_id'], row['customer_id'], row['employee_id'], row['order_date'], row['ship_city']))  # Заполнение данных в таблицу

conn.clouse()  # Закрываем подключение к БД
